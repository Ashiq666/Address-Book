...
public class HandleDB extends SQLiteOpenHelper {
    ...
    private void createKeyValueTable(SQLiteDatabase db){
        try {
            db.execSQL("create table " + TABLE_KEY_VALUE + " (" + KEY
                    + " TEXT, " + NAME + " TEXT, " + EMAIL + " TEXT, " + PHONE + " TEXT, " + ADDRESS + " TEXT)");
        } catch (Exception e) {
        }
    }
    @Override
    ...
    public Boolean insertKeyValue(String key, String name, String email, String phone, String address) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public boolean updateValueByKey(String key, String name, String email, String phone, String address) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(KEY, key);
        cv.put(NAME, name);
        ...
        if (nr == 0) {
            insertKeyValue(key, name, email, phone, address);
        }
        return true;
    }
